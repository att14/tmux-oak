...
func renderTree(nodes []TreeNode, cursor int, expanded map[int]bool, width int) string {
	...
	for i, node := range nodes {
		line := renderNode(node, expanded, width)
		if i == cursor {
...
func renderNode(node TreeNode, expanded map[int]bool, width int) string {
	switch node.Kind {
	case WindowNode:
		return renderWindowNode(node, expanded, width)
	case PaneNode:
		return renderPaneNode(node)
	}
	return ""
}
...
func renderPaneNode(node TreeNode) string {
	p := node.Pane
	connector := "├"
	if node.IsLastChild {
		connector = "└"
	...
	label := fmt.Sprintf("   %s %s", connector, p.Command)

	if p.Active {
		return paneActiveStyle.Render(label)
	}
	return paneDimStyle.Render(label)
}
